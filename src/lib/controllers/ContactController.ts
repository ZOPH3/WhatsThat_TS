...
const ContactsController = (): IContactsController => {
  ...
  const fetchContactList = async (): Promise<TUser[] | undefined> => {
    const response = await useApi({ url: '/contacts', method: 'GET' }, true);
    return response.data as TUser[];
  };

  const addContact = async (user_id: number): Promise<Response | undefined> => {
    const response = await useApi({ url: `/user/${user_id}/contact`, method: 'GET' }, true);
    return response.data as Response;
  };

  const deleteContact = async (user_id: number): Promise<Response | undefined> => {
    const response = await useApi({ url: `/user/${user_id}/contact`, method: 'DELETE' }, true);
    return response.data as Response;
  };

  const blockUser = async (user_id: number) => {
    const response = await useApi({ url: `/user/${user_id}/block`, method: 'GET' }, true);
    return response.data as Response;
  };

  const unblockUser = async (user_id: number): Promise<Response | undefined> => {
    const response = await useApi({ url: `/user/${user_id}/block`, method: 'DELETE' }, true);
    return response.data as Response;
  };

  const fetchBlockedList = async (): Promise<TUser[] | undefined> => {
    const response = await useApi({ url: '/blocked', method: 'GET' }, true);
    return response.data as TUser[];
  };
  ...
  const searchUsers = async (parameters: TSearchParams): Promise<TUser[] | undefined> => {
    ...
    const response = await useApi({ url: base, method: 'GET' }, true);
